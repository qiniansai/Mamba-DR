...
def selective_scan(u, delta, A, B, C, D, z=None, delta_bias=None,
    ...
    """按设备分发：CUDA 走编译 kernel，CPU 走 mamba_ssm 自带参考实现（数值一致）。"""
    if u.is_cuda:
        return selective_scan_fn(u, delta, A, B, C, D, z=z,
                                 delta_bias=delta_bias,
                                 delta_softplus=delta_softplus,
                                 return_last_state=return_last_state)
    return selective_scan_ref(u, delta, A, B, C, D, z=z,
                              delta_bias=delta_bias,
                              delta_softplus=delta_softplus,
                              return_last_state=return_last_state)
...
class MambaVisionMixer(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        B_flip = torch.flip(B, dims=[-1])
        C_flip = torch.flip(C, dims=[-1])
        
        y_backward = selective_scan(x_flip,
                              dt_flip,
                              A,
                              B_flip,
                              C_flip,
        ...
        y_fused = self.fusion_ffn(rearrange(torch.cat([y_forward, y_backward], dim=1), "b d l -> b l d"))
        y_fused = rearrange(y_fused, "b l d -> b d l")
        
        y = torch.cat([y_fused, z], dim=1)
