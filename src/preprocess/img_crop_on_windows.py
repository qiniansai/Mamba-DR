...
# fixed some bugs by Zhao Yutao 2026.1.30 on Windows 11

IMG_SIZE = 512
NUM_WORKERS = 8  # according to your CPU cores
FUNDUS_PATH = glob(
    "./DDR-dataset/lesion_segmentation/*/image/*.jpg"  # set your own path
)
MASK_PATH = lambda file_name: glob(  # noqa: E731
    f"./DDR-dataset/lesion_segmentation/*/label/*/{file_name}.tif"
)  # set your own path
...
def process_single_image(fundus_path):
    ...
    for mask_img, mask_path in zip(mask_imgs, mask_paths_sameid):
        ...
        # 步骤1：获取掩码路径的父目录（即病变类别所在目录）
        mask_parent_dir = os.path.dirname(mask_path)  # 例：Windows下得到 C:\data\label\hemorrhage
        # 步骤2：提取父目录的名称（即病变类别名）
        lesion_name = os.path.basename(mask_parent_dir)  # 正确提取 hemorrhage
